...
"""
Rutas de Administración Premium
-------------------------------
...
def list_users(request: Request, db: Session = Depends(get_db), current_user: User = Depends(get_current_user)):
    ...
    try:
        with tracing.start_span("admin:list_users"):
            ...
            logger.info(f"[ADMIN] Listado de usuarios solicitado | total={len(users)} by={current_user.email} ip={ip}")
            metrics.record_request("GET", "/admin/users")
            metrics.record_latency("/admin/users", time.time() - start)
            return users
    except Exception as e:
        metrics.record_error("/admin/users", severity="CRITICAL")
        alerts.send_alert(f"Error al listar usuarios: {e}", severity="CRITICAL")
        logger.error(f"[ADMIN] Error al listar usuarios: {e} | ip={ip}")
...
def delete_user(user_id: int, request: Request, db: Session = Depends(get_db), current_user: User = Depends(get_current_user)):
    ...
    try:
        with tracing.start_span("admin:delete_user"):
            ...
            db.delete(user)
            db.commit()
            logger.warning(f"[ADMIN] Usuario eliminado | id={user_id} by={current_user.email} ip={ip}")
        ...
        db.rollback()
        metrics.record_error("/admin/user/{user_id}", severity="CRITICAL")
        alerts.send_alert(f"Error al eliminar usuario {user_id}: {e}", severity="CRITICAL")
        logger.error(f"[ADMIN] Error al eliminar usuario {user_id}: {e} | ip={ip}")
